...
namespace AspNet.Dev.Pkg.Infrastructure.Controller
{
    ...
    where CreateT : BaseCreate
    {
        ...
        public QueryController(IQueryApplication<T, Return, GetT, CreateT> app) : base(app) => _app = app;

        [HttpGet]
        public virtual async Task<PageData<Return>> FindPageAsync([FromQuery] GetT input, [FromQuery] PageRequest page) => await _app.FindPageAsync(input, page);

        [HttpGet, Route("Query")]
        public virtual async Task<ICollection<Return>> FindQueryAsync([FromQuery] GetT input) => await _app.FindQueryAsync(input);
    }
    [Route("[controller]")]
    ...
    where CreateT : BaseCreate
    {
        ...
        public QueryController(IQueryApplication<T, GetT, CreateT> app) : base(app) => _app = app;
        /// <summary>
        /// 带条件分页
        /// </summary>
        [HttpGet]
